...
// 静态方法调用
TopDialog.open(props: DialogProps): TopViewManager
```
...
| TopDialog.open() | Dialog.show() | 静态方法调用方式变更 |
| - | bottomText | 新增底部提示文本 |
| - | scrollable | 新增滚动支持 |
...
```tsx
// 迁移前
import { TopDialog } from '@mtfe/empower-mrn-components/shuguopai'
...
### 案例 2：静态方法调用 - TopDialog.open()

```tsx
// 迁移前
import { TopDialog } from '@mtfe/empower-mrn-components/shuguopai'

const topViewManager = TopDialog.open({
  title: '提示',
  message: '操作成功',
  buttons: [
    { text: '我知道了', type: 'confirm', onPress: () => topViewManager.close() }
  ],
  onRequestClose: () => topViewManager.close()
})
...
<TopDialog
  ...
  message="自定义遮罩效果"
  maskOpacity={0.8}
  maskStyle={{ backgroundColor: '#000000' }}
  buttons={[
    { text: '确认', type: 'confirm', onPress: () => setShow(false) }
  ]}
/>

// 迁移后 - 新组件不支持自定义遮罩透明度和样式，使用默认值
import { Dialog } from '@sfe/wand-rn'
...
<TopDialog
  ...
  layout="vertical"
  buttons={[
    { text: '操作一', type: 'confirm', onPress: () => handleAction1() },
    { text: '操作二', type: 'confirm', onPress: () => handleAction2() },
    { text: '取消', type: 'cancel', onPress: () => setShow(false) }
  ]}
/>

// 迁移后 - 新组件不支持垂直布局，需调整设计或使用自定义内容
import { Dialog } from '@sfe/wand-rn'
...
<Dialog
  ...
  content={
    <View>
      <Text style={{ marginBottom: 16 }}>请选择要执行的操作</Text>
      <TouchableOpacity onPress={() => handleAction1()} style={styles.actionButton}>
        <Text>操作一</Text>
      </TouchableOpacity>
      <TouchableOpacity onPress={() => handleAction2()} style={styles.actionButton}>
        <Text>操作二</Text>
      </TouchableOpacity>
    </View>
  }
...
1. **组件使用方式**:
   - JSX 方式: `TopDialog` → `Dialog`
   - 静态方法: `TopDialog.open()` → `Dialog.show()`
   - 静态方法返回值: `TopViewManager` → `number`（Portal key）
...
5. **移除的功能**:
   ...
   - 不支持自定义 `maskOpacity`
   - 不支持 `TopViewManager.close()` 方法手动关闭（改用状态控制）
...
2. **静态方法调用差异**: 
   - 旧组件的 `TopDialog.open()` 返回 `TopViewManager`，可调用 `.close()` 手动关闭
   - 新组件的 `Dialog.show()` 返回 `number`（Portal key），无法手动关闭，需通过状态控制

3. **样式一致性**: 新旧组件的默认样式可能不同，迁移后需进行视觉验收

4. **按钮默认类型**: 注意按钮的 `type` 默认值从 `'confirm'` 变为 `'cancel'`，需显式指定
...
9. **TopDialog vs Dialog**: 在 shuguopai 包中，`TopDialog` 是基于 `@ss/mtd-react-native` 的 Modal 封装的组件，与普通 `Dialog` 的主要区别是底层实现方式，但 API 基本一致。迁移到 `@sfe/wand-rn` 后统一使用 `Dialog` 组件即可。
