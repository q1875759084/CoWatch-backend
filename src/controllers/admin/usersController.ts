...
export const AdminUsersController = {
  ...
  list(_req: Request, res: Response): void {
    ...
    const users = rows.map((u) => ({
      ...
      isBanned: (u as unknown as Record<string, number>).is_banned === 1,
      plans: getActivePlans(u.id),
   ...
   */
  setBan(req: Request, res: Response): void {
   ...
   */
  grantPlan(req: Request, res: Response): void {
    const { userId } = req.params;
    ...
    addSubscription(userId, plan, expiresAt);
