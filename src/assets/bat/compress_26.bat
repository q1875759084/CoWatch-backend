...
:: CoWatch Video Compressor - Balanced (CRF 26)
...
echo.
echo  ==========================================
echo   CoWatch Compressor - Balanced (CRF 26)
echo  ==========================================
echo   Input : %INPUT%
echo   Output: %OUTPUT%
echo  ------------------------------------------
...
:run_encode
echo  ------------------------------------------
echo   Quality: CRF 26 / CQ 26, ~1 GB per 30 min
...
:: -y 自动覆盖同名输出文件
if "%ENCODER%"=="h264_nvenc" "%FFMPEG_CMD%" -i "%INPUT%" -c:v h264_nvenc -cq 26 -preset p2 -pix_fmt yuv420p -c:a aac -b:a 128k -movflags +faststart -g 300 -keyint_min 300 -y "%OUTPUT%"
if "%ENCODER%"=="h264_qsv"  "%FFMPEG_CMD%" -i "%INPUT%" -c:v h264_qsv -q:v 26 -preset veryfast -pix_fmt yuv420p -c:a aac -b:a 128k -movflags +faststart -g 300 -keyint_min 300 -y "%OUTPUT%"
if "%ENCODER%"=="h264_amf"  "%FFMPEG_CMD%" -i "%INPUT%" -c:v h264_amf -quality speed -qp_i 26 -qp_p 26 -qp_b 28 -pix_fmt yuv420p -c:a aac -b:a 128k -movflags +faststart -g 300 -keyint_min 300 -y "%OUTPUT%"
if "%ENCODER%"=="libx264"   "%FFMPEG_CMD%" -i "%INPUT%" -c:v libx264 -crf 26 -preset veryfast -pix_fmt yuv420p -c:a aac -b:a 128k -movflags +faststart -g 300 -keyint_min 300 -sc_threshold 0 -y "%OUTPUT%"
